...
b.Hub()                   // → *sse.Broadcaster[sse.Event] (access SubscribeFilter, share hub)
```
...
`Hub()` exposes the underlying `*sse.Broadcaster[sse.Event]` so consumers can access advanced go-sse features (`SubscribeFilter`, custom health) or share one fan-out hub across HTMX and Datastar transports (`Raw()`/`NewBroadcasterFromRaw` are deprecated aliases, v5 removal):

```go
hub := sse.NewBroadcaster[sse.Event]()
htmxBc := cqrshtmx.NewBroadcasterFromHub(hub)
dsBc := ds.NewBroadcasterFromHub(hub) // datastar.Broadcaster from same hub

// Broadcast from either wrapper reaches ALL subscribers
htmxBc.Broadcast(sse.Event{Event: "update", Data: html})
```

Both `*cqrshtmx.Broadcaster` and `*datastar.Broadcaster` embed the hub, so `Subscribe`/`SubscribeFilter`/`OnSubscribe` promote directly. See `docs/guides/sse-and-datastar.md`.
