...
func (f *fakeStreamReader) List(_ context.Context, _ listing.ListOptions) (*listing.Page[listing.StreamListing], error) {
	return &listing.Page[listing.StreamListing]{Items: f.items}, nil
}
...
func (f *fakeStreamReader) ListWithStatus(_ context.Context, _ listing.ListOptions) (*listing.Page[listing.StreamStatus], error) {
	...
	for i, item := range f.items {
		statuses[i] = listing.StreamStatus{Ref: item}
	}
	return &listing.Page[listing.StreamStatus]{Items: statuses}, nil
}
...
func TestTimeTravelIndexHandler_WithListings(t *testing.T) {
	...
	body := w.Body.String()
	if !strings.Contains(body, "Inspect aggregate at any point") && !strings.Contains(body, "Time Travel") {
		t.Fatalf("expected time-travel heading, got: %s", body[:min(200, len(body))])
	}
	...
	if !strings.Contains(body, "/time-travel/user/") {
		t.Fatalf("expected time-travel detail link, got: %s", body[:min(200, len(body))])
	}
}
...
func TestSnapshotsIndexHandler_WithListings(t *testing.T) {
	...
	if !strings.Contains(body, "/snapshots/user/") {
		t.Fatalf("expected snapshot detail link, got: %s", body[:min(200, len(body))])
	}
}
