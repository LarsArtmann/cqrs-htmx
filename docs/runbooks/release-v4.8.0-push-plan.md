...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `metaengine/projectionadapter/v4 v4.5.0` | `2e9a2fc28` (re-validated 2026-08-15 against the post-ADR-0128 tree; any later commit containing `typed_decoder.go` works) | **Strip the workspace `replace (...)` block** from `metaengine/projectionadapter/go.mod` before tagging — its requires (event v4.6.0, id v4.4.0, metaengine v4.10.0, projection v4.3.0, projectionhost v4.0.0, record v4.2.0 — all confirmed on the go-cqrs-lite origin 2026-08-15 — plus otel and external `go-codec v0.1.0`) are all published. |
| `metaengine/sqliteengine/v4 v4.0.2`      | `2e9a2fc28`                                                                                                                | **Strip** `replace metaengine/v4 => ../` from `metaengine/sqliteengine/go.mod`; requires metaengine v4.10.0 + record v4.2.0 (published, confirmed on origin).                                                                                                                                                                                     |
...
8. `datastar`, `examples/datastar-demo` — strip go-datastar replaces after §3 go-datastar tags. **DONE 2026-08-15 (complete): upstream replaces stripped in `datastar`/`examples/datastar-demo`/`integration_test`/`health` (`f128072d`); signed `datastar/v4.8.0` tag cut AND pushed (user); tag confirmed resolvable; the 2 datastar-local family replaces stripped from `examples/datastar-demo` + `integration_test` with hermetic tidy/build/vet green each (`b5663b2b`). Zero datastar replaces remain anywhere.**
