...
- **Against extraction**: The rejection *code* varies per domain, so the helper would need a `domain` parameter. The abstraction would be ~3 lines, saving ~2 lines per site. The variation (rejection code) is the meaningful part, not the shared shape.
