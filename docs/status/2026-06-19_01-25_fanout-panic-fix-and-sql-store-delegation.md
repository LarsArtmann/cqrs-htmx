...
- **Remaining (all triaged, all opinionated or false positives)**:
  ...
  - **27 mixins** — low-confidence 2-field overlaps (e.g., `ImportUser`/`ExportUser` where `Email` is a *different type* in each).
...
- **Prior fuckup (now fixed)**: The `fanout.Broadcast()` snapshot optimization that was documented in AGENTS.md as a *performance improvement* was actually a **concurrency bug** that could crash the process. It was there for months. The "optimization" traded 1 allocation for a race condition. Fixed in this session.
