...
25. setup-demo: return error from `seed()` instead of `log.Fatalf` (gocritic exitAfterDefer). → routed: ROADMAP setup surface ideas (2026-09-09).
26. setup-demo e2e: assert authenticated `/dashboard/` render (currently only the 401 gate + admin flow are asserted).
27. Add godoc Example functions for `Run`/`RunHandler` (compile-checked doc snippets). → routed: ROADMAP setup surface ideas (2026-09-09).
...
34. Cross-link `docs/guides/async-projection-startup.md` from fullstack-wiring (Run + AsyncStartup compose naturally). → routed: ROADMAP residual micro-ideas (2026-09-09).
