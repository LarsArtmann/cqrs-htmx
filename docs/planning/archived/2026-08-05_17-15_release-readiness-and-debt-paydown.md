...
| M13 | **Auto-discover modules from go.work** — replace hardcoded lists in flake.nix apps with `go work edit -json                                                                                                        | jq` dynamic discovery; requires architecture decision for shell-in-Nix pattern | MEDIUM | 60min       | none     |
...
| M16 | __Wire remaining check-_ into CI_* — check-codegen (needs templ pinning solution), check-templates (needs workspace-mode step)                                                                                     | LOW                                                                            | 45min  | M05         | P2       |
...
| F67 | M13    | Prototype `go work edit -json                                                                                | jq -r '.Mapping[].DiskPath'` in nix develop |
