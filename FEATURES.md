...
| Feature              | Status                    | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Offline Command Sync | 🟡 `PARTIALLY_FUNCTIONAL` | `SyncWorkerHandler()` serves the embedded SharedWorker (`sync/sync-worker.js`); `SyncClientHandler()` serves the tab-side client (`sync/sync-client.js`). The SharedWorker queues failed HTMX command envelopes to IndexedDB when the network is down, drains on spawn for cross-session retry, deletes on ACK, and degrades to in-memory when IDB unavailable. Retry count (max 10) + TTL (24h) eviction. Staggered, targeted retry delivery. `SyncClientScriptTag(path)` helper. ADR-0042. Not yet E2E-browser-tested. |
