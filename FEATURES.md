...
| Offline Sync        | 🟡 `PARTIALLY_FUNCTIONAL` | SharedWorker (`sync-worker.js`) queues failed commands with **IndexedDB persistence (Phase 2b, ADR-0040)**: drains on spawn for cross-session retry, deletes on ACK, degrades to in-memory when IDB unavailable. `rebuildAndRetry` reconstructs lost DOM targets via synthetic host + `htmx.ajax`. Browser behavior is unit-test-verified but not yet E2E-tested in a real browser. |
