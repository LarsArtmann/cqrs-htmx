...
| 22  | Correlation ID            | FULLY_FUNCTIONAL | `WithCorrelationID(ctx, id)` / `CorrelationIDFromContext(ctx)`. Auto-extracted from `X-Correlation-ID` header in `ContextEnrichmentMiddleware`.                                                                                                                            |
