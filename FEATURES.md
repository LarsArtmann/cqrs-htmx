...
| Offline Sync        | 🟡 `PARTIALLY_FUNCTIONAL` | SharedWorker (`sync-worker.js`) queues failed commands with **IndexedDB persistence (Phase 2b, ADR-0040)**: drains on spawn for cross-session retry, deletes on ACK, degrades to in-memory when IDB unavailable. Retry count (max 10) and TTL (24h) prevent poison commands. Staggered, targeted retry delivery (originating tab preferred, round-robin fallback) prevents thundering herd. `rebuildAndRetry` reconstructs lost DOM targets via synthetic host + `htmx.ajax`. Tab tracking via hello/bye protocol with dead-port cleanup. Browser behavior is unit-test-verified but not yet E2E-tested in a real browser. |
