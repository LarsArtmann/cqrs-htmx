...
**Updated:** 2026-07-31 | **Version:** v4.6.1 (go-cqrs-lite v4.2.0; see AGENTS.md for per-sub-module versions) | **Lint:** 0 issues across all 15 modules | **Coverage gates:** root 93.6% (gate 90%), usermgmt 81.7% (gate 74%), identity-model 74.9% (gate 70%), dashboardui 82.1% (gate 60%)
...
- **Coverage:** 93.7% root, 80.9% usermgmt, 74.9% identity-model (gate 70%), 88.2% totp, 89.2% webauthn, 88.3% oauth2, 69.0% adminui, 80.1% loginpage, 78.7% dashboardui (gate 60%). Race-safe. CI gates: root 90%, usermgmt 74%, identity-model 70%, auth 80%, adminui 66%, loginpage 79%, dashboardui 60% (see `nix run .#coverage-gate`). All 9 modules have coverage gates.
...
| Phantom-version CI gate       | Add a CI step that detects broken zero pseudo-versions (`v4.0.0-00010101000000-000000000000`) in workspace `go.mod` files. This has been a recurring problem across 10+ sessions. A simple `grep -r "00010101000000-000000000000" --include="go.mod"` check in `.buildflow.yml` would catch it pre-merge. | ~15 min |
