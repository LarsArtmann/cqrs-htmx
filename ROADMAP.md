...
4. ~~**httputil `ContentTypeNosniff` vs `ContentTypeOptions`.**~~ **Resolved:** httputil v0.11.0 is published with both fields. `ContentTypeOptions string` coexists with the older `ContentTypeNosniff bool`; the `SecurityHeaderSkip` sentinel (`"-"`) short-circuits correctly (bug fixed in v0.9.0). cqrs-htmx depends on v0.11.0 and the go.work replace is removed.
