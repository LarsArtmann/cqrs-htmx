...
12. **systemadapter first-tag version number.** When the upstream `metaengine/projectionadapter/v4 v4.5.0` blocker clears: tag systemadapter at the then-current family version (v4.10.0+) or at an independent v4.8.0 (its recommended first-version on record, docs/status/2026-08-30_21-00 g2)? Related decision: build or reject the `setup.NewFromSystem()` bridge (asked 2026-08-09 ×2).
