...
- **Version:** v4.5.0 (15 modules: root + identity-model + usermgmt + 3 auth sub-modules + adminui + loginpage + dashboardui + integration_test + 5 examples). Released 2026-07-24; inter-module version refs resolved to clean tags (`e274540`) — only `examples/dashboard-demo/go.mod` retains a zero pseudo-version (see TODO_LIST P1).
- **Coverage:** 93.5% root, 81.0% usermgmt, ~41% identity-model (no gate yet), 88.2% totp, 89.2% webauthn, 88.3% oauth2, 69.0% adminui, 80.1% loginpage, dashboardui low (~960 tests total), race-safe. CI gates: root 90%, usermgmt 74%, auth 80%, adminui 66%, loginpage 80% (see `nix run .#coverage-gate`).
- **Lint:** 0 issues across all linted submodules. Root carries ~80 pre-existing low-severity style nits (varnamelen ×50, staticcheck SA1019 ×18, testpackage ×9) — not gate-blocking.
- **ErrorFamily:** 0 violations across all modules.
- **Dependencies:** go-cqrs-lite v4.0.x (sub-modules v4.0.0–v4.0.2; workspace references up to v4.1.0), go-error-family v0.8.0, go-branded-id v0.3.2, go-sse v0.2.0, httputil v0.5.0, templ-components v0.16.0. Casbin v3 is a first-class dependency of identity-model (ADR-0044). Auth deps (go-webauthn, oauth2, oidc, pquerna/otp) are in optional sub-modules — core usermgmt has ZERO auth deps.
