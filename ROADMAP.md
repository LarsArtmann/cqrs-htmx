...
- **Lint:** `nix run .#lint` currently fails on root (~160), usermgmt (~100), and dashboardui (~150) on pre-existing style nits (varnamelen, exhaustruct, `mnd`, canonical header casing) and `id.*AggregateID` SA1019 deprecation warnings; the other 7 modules pass clean. Non-release-blocking (v4.5.0 and v4.6.0 shipped with them). The wrapper stops at the root failure, so submodule failures are only visible via `GOEXPERIMENT=jsonv2 golangci-lint run` per module.
...
| Bus  | `event.Bus.UnsubscribeAll` / context-cancellable `SubscribeAll`    | Low      | **Open** — `dashboardui.Dashboard.Close()` cannot fully unsubscribe its bus handler (no removal API), leaking the handler closure. Workaround: context-cancellable wrapper (TODO_LIST P1). Upstream PR pending. |
