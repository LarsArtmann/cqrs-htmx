...
declare -A DEP_BUDGET=(
  ...
  ["setup"]=21            # setup: 21 current (cqrs-htmx family x7 [root/usermgmt/identity-model/adminui/dashboardui/loginpage/datastar], cqrs-lite event/id/projectionhost/storage-memory/watermill/eventtest/storage, go-sse [direct since /sse replay], go-appkit [RunWithAppkit], error-family, httputil, modernc.org/sqlite [ADR-0050 wave]; 2026-09-14 OTel sprint added middleware/v4 [production: Config.Observability accepts *middleware.OTelBundle] + otel/v4 [test-only: tracer for wiring tests])
