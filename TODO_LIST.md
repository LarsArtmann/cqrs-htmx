...
- [ ] **usermgmt Sollbruchstellen extraction** (v3-safe + v4-breaking) — 84 prod files / ~11K LOC in one flat `package usermgmt`. Three clean seams identified in `docs/modularization/2026-07-01_SOLLBRUCHSTELLEN.html`: **(v3-safe)** extract `usermgmt/es` (20 pure domain files), `usermgmt/sql` (9 SQL infra files) as sub-packages — no API breakage. **(v4-breaking)** extract `usermgmt/totp`, `usermgmt/webauthn`, `usermgmt/oauth2` behind interfaces (TOTPVerifier, WebAuthnProvider, OAuth2Provider) — removes go-webauthn, oauth2, oidc, jose, pquerna/otp from consumer deps
- [ ] **Root module Sollbruchstellen extraction** (v3-safe) — 16 of 46 root files have ZERO logic coupling to core. 7 clean sub-package candidates: `cqrshtmx/sse` (5 files), `cqrshtmx/ws` (4 files), `cqrshtmx/ratelimit` (2 files), `cqrshtmx/servertiming` (1 file, 360 lines!), `cqrshtmx/security` (1 file), `cqrshtmx/htmxjs` (2 files). Also: move 5 shared constants to `constants.go` to break the errors↔response↔csrf "cycle" (which is NOT structural — just string constants). See `docs/modularization/2026-07-01_SOLLBRUCHSTELLEN.html`
