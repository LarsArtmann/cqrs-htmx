...
- [ ] **Store-layer upcasting for the projection path via `go-cqrs-lite/schema`.** identity-model's `UpcasterRegistry` upcasts only at decode time (fold functions / read models). Projections reading raw events from a `SeekableJournal` via `projectionhost` bypass those upcasters. Wrap the projection host's journal with `schema.VersionedSeekableJournal` so upcasting runs consistently at the store boundary. Source: `docs/guides/leveraging-go-cqrs-lite.md` §9. Evidence: `identity-model/upcaster.go:108` (`applyUpcasters`) is decode-time only.
