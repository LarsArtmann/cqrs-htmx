...
**Updated:** 2026-08-14 | **Version:** v4.7.0 (released 2026-08-07) + `[Unreleased]` (async projection startup + ADR-0048, ActorID consolidation/ADR-0111, ADR-0114 tombstone migration, MySQL event-store completion, security middleware consolidation, httputil v0.11.0, Broadcaster Raw() accessor, fullstack UI integration test, systemadapter module + system/metaengine integration, hermetic-build repair) | **Modules:** 24 in `go.work` (13 production + 10 examples + e2e/server) | **`*Service` methods:** 73 (leading v5 indicator; see ROADMAP) | **Coverage (verified 2026-08-14, all gates green):** Root 93.4% (gate 90%), openapi 99.0%, usermgmt 80.8% (gate 74%), identity-model 75.5% (gate 70%), dashboardui 83.4% + core 86.1% (gates 60/80), datastar 97.5% (gate 90%), setup 89.5% (gate 80%), systemadapter 89.6% (gate 70%), adminui 68.5% (gate 66), loginpage 79.9% (gate 79) | **Lint:** 0 golangci issues across 13 modules incl. systemadapter (2026-08-14); cqrs-lint strict passes all module configs. | **⚠️ Temporary local replaces pending upstream tags:** identity-model v4.7.0+ (5 modules), metaengine/projectionadapter v4.5.0+ (2), metaengine/sqliteengine v4.0.2+ (1) — see AGENTS.md Gotchas.
...
- [ ] **Tag pending module releases and strip temporary local replaces.** `identity-model/v4 v4.7.0` is referenced by usermgmt/adminui/setup/integration_test/examples-setup-demo go.mod files but NOT tagged — 5 temporary local replaces keep hermetic builds working. Same for `metaengine/projectionadapter` (post-v4.4.0 `EventWithID.OccurredAt`) and `metaengine/sqliteengine` (post-v4.0.1 `register.go` driver self-registration) in systemadapter/system-demo. Each replace carries a removal-condition comment; strip them all after tagging (verify GOWORK=off builds resolve from the proxy).

---

## P2 — Medium impact (tooling & quality)

- [~] **`systemadapter/v4` remaining gate integration.** Lint remediation DONE (2026-08-14: 0 golangci issues after `OnTyped`→`OnRecordTyped` migration, errorfamily constructors, `WithCheckpointStore`/`WithDeadLetterStore` options; module included in `nix run .#lint`, cqrs-lint strict clean, coverage-gate 89.6%/70%). Remaining: `check-module-isolation.sh` + `check-dep-budgets.sh` wiring, verify CI workflow runs its gates.
