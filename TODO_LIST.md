...
- [ ] **Adopt appkit as the `setup` server layer (ADR-001 Option A confirmed by spike).** The `spike/appkit-server` branch carries `RunWithAppkit` + passing SSE-flush/readiness/parity tests and the adopt report (`docs/status/2026-08-16_01-38_appkit-spike-adopt.md`): benchmark 16178 vs 45049 ns/op (delta is per-request INFO logging; ~22k req/s remains), drain-phase and projection-aware readiness both verified under `-race`. BLOCKED on go-appkit publishing a tag with `NoTimeout` + `ReadyCheck` (core ≥ v0.3.0); then: drop the spike `replace`, fold `RunWithAppkit` into `RunHandler` behind the same signature, and consider exposing `Addr()` (new capability).
