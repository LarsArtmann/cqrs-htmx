...
- **Root module is intentionally a single flat package**: 46 files form a cohesive "HTMX-aware CQRS HTTP integration" library. The errors↔response↔csrf 3-node cycle (confirmed 2026-07-01) prevents extraction. Sub-package extraction would harm consumer UX. See `docs/modularization/2026-07-01_PROPOSAL.html` for full analysis
- **usermgmt god-package debt** (2026-07-01 review): 84 prod files / ~11K LOC in one flat `package usermgmt`. `Service` struct directly holds `*webauthn.WebAuthn` + `oauth2Providers`, forcing go-webauthn, oauth2, oidc, jose, otp, sqlite as transitive deps on ALL consumers. Unix pipe test FAILS. Fix: extract `usermgmt/{webauthn,oauth2,totp,sql}` sub-packages with interface-based registration. Breaking change — deferred to v4. See `docs/modularization/2026-07-01_PROPOSAL.html`
