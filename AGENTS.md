...
├── htmx_serve.go     # HTMXScriptHandler, HTMXScriptHandlerWith (custom JS), HTMXCDNScriptTag, HTMXScriptTag, serveJS helper
├── htmx_extensions.go # Embedded HTMX extensions (sse/ws/idiomorph), HTMXExtensionHandler, HTMXExtensionsHandler (bundle)
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| go-cqrs-lite v4.0.0          | CQRS dispatch, pagination, event sourcing (decider, storage/memory, watermill bus, SQL view stores, typed metadata) | All modules                  |
...
- **Root module flat package with 10 Sollbruchstellen** (2026-07-01 v3 review): 46 production files. The errors↔response↔csrf "cycle" is NOT structural — it's 5 string constants in response.go. Moving them to constants.go breaks it. 16 of 46 files (35%) have ZERO logic coupling to core (SSE/WS/ratelimit/security/servertiming/fanout/htmxjs). 7 clean sub-package extraction candidates identified. See `docs/modularization/2026-07-01_SOLLBRUCHSTELLEN.html` for full analysis
...
- **Root → usermgmt: zero imports** (clean boundary). **usermgmt → root: YES** (RateLimiter). usermgmt imports `cqrs-htmx/v4` for `RateLimiter` (rate limiting unification). This is a one-way dependency — root never imports usermgmt. Cross-module bridging tests happen in `integration_test/`
...
- **go-error-family v0.7.0**: Replaced `cockroachdb/errors` for error classification. Now a DIRECT dependency in root + usermgmt + all auth strategy modules (was indirect via event/v3). Re-exported via `go-cqrs-lite/event/v4` (`event.NewRejection`, `event.WrapTransient`, `event.Classify`, etc.). `ErrDispatchFailed` is natively classified (`event.NewTransient`) — the old `sync.Once` + `RegisterClassification` machinery was removed
...
- **HTMX v2.0.10 minified**: Embedded via `//go:embed htmx.min.js` in `htmx_embed.go` (~51KB). No CDN dependency — consumers serve it from their Go binary
...
- **HTMXVersion()**: Returns `"2.0.10"` — useful for cache-busting query params
...
- **3 extensions embedded** in `extensions/` dir, served via `htmx_extensions.go`: SSE (htmx-ext-sse 2.2.4), WS (htmx-ext-ws 2.0.4), idiomorph (0.7.4). These are the extensions with direct server-side counterparts in cqrs-htmx (SSEStream/Broadcaster/WSMessage/WSBroadcaster) or commonly paired with them (idiomorph morph-swap for SSE partial updates — PR #3870 version bug fixed by pinning ≥ 2.2.4)
- **HTMXExtensionHandler(name)**: Serves a single embedded extension by name. Constants: `HTMXExtSSE` ("sse"), `HTMXExtWS` ("ws"), `HTMXExtIdiomorph` ("idiomorph"). Panics on unknown name. Same caching headers as HTMXScriptHandler (ETag, Cache-Control 1yr immutable, 304 support)
- **HTMXExtensionsHandler(names...)**: Serves a concatenated bundle — one HTTP request instead of N. Extensions prefixed with version comments. Composite ETag includes all names+versions
- **HTMXExtensionVersion(name)**: Returns version string (e.g. `HTMXExtensionVersion(HTMXExtSSE)` → `"2.2.4"`). Empty string for unknown
- **HTMXExtensionNames()**: Returns all available extension names sorted alphabetically
- **Usage**: `mux.Handle("GET /ext/sse.js", cqrshtmx.HTMXExtensionHandler(cqrshtmx.HTMXExtSSE))` or `mux.Handle("GET /ext/bundle.js", cqrshtmx.HTMXExtensionsHandler(cqrshtmx.HTMXExtSSE, cqrshtmx.HTMXExtWS, cqrshtmx.HTMXExtIdiomorph))`
- **Not auto-wired**: Consumer registers routes and adds `<script>` tags in their layout. Extensions require htmx core to be loaded first
...
- **Idempotency**: `idempotency.go` is now thin backward-compatible aliases over `go-cqrs-lite/idempotency/v4` (now at v4.0.0). Re-exports `IdempotencyStore` (= `idempotency.Store`), `MemoryIdempotencyStore` (= `idempotency.MemoryStore`), `ErrDuplicateCommand` (= `idempotency.ErrDuplicate`), and `NewMemoryIdempotencyStore` wrapper. `CheckAndRecord(ctx, key, ttl)` atomicity (single lock in-memory, future `SET NX` for Redis, `INSERT ON CONFLICT` for SQL). `ErrDuplicateCommand` maps to HTTP 409 Conflict. NOT auto-wired — consumers opt in via `BeforeDispatchHook` or middleware. See `docs/adr/0026-command-idempotency-store.md`.
...
- **usermgmt → root dependency**: usermgmt now imports `cqrs-htmx/v4` for `RateLimiter`. This is a one-way dependency — root never imports usermgmt. The old claim of "zero mutual imports" is obsolete.
...
- **Embedded WebAuthn JS** (`assets/login.js`): The JS handles the full ceremony: (1) Base64URL ↔ ArrayBuffer conversion (identical in every consumer), (2) Login flow: POST `/auth/webauthn/login/begin` → `navigator.credentials.get()` → serialize assertion → POST `/auth/webauthn/login/finish` → redirect, (3) Registration flow: POST `/auth/register` (server auto-generates user ID) → read `user.id` from response → POST `/auth/webauthn/register/begin` → `navigator.credentials.create()` → serialize attestation → POST `/auth/webauthn/register/finish` → redirect, (4) Browser detection: checks `window.PublicKeyCredential` before showing WebAuthn form; shows fallback message for unsupported browsers, (5) Error handling: `NotAllowedError` → "cancelled", `SecurityError` → "domain not authorized", network errors → "could not reach server", RFC 7807 `title`/`detail`/`status` extraction from API errors.
...
- **Config**: `{Service (required), Title, Brand, Redirect, AccentColor, CSSPath, NoRegistration, AuthPrefix, OAuth2Buttons, CredentialName}`. `AuthPrefix` remaps API endpoint paths (default `/auth/...`, set to `/api` for `/api/auth/...`). `OAuth2Buttons` can be manually set or auto-populated from `Service.ConfiguredOAuth2Providers()`. `CredentialName` is the label stored with new WebAuthn credentials (default "Passkey").
- **Server-side ID generation**: `RegisterRequest.ID` is now optional — when empty, `Service.Register()` auto-generates a ULID via `id.NewUserID()`. Existing callers that provide an ID still work (backward compatible). login.js reads the server-returned `user.id` from the register response instead of generating `crypto.randomUUID()`.
- **Accessibility**: `aria-live="polite"` on error div for screen reader announcements.
- **Tests**: stdlib `testing` + `httptest` — 30+ tests covering config validation, page rendering, CSRF injection, endpoint URLs, registration visibility, accent color, CSS path, auth prefix, safe redirects, browser fallback, favicon, OAuth2 auto-detection, display names, HEAD method, and render error paths. Coverage 80.1%.
...
- **Service.HasWebAuthn/HasOAuth2/HasTOTP/ConfiguredOAuth2Providers**: Exported methods on `usermgmt.Service` (added in `service_auth_methods.go`). `ConfiguredOAuth2Providers()` returns sorted provider names via type assertion on the `OAuth2Provider` interface — backward compatible with custom implementations that don't implement `Names()`. loginpage auto-populates OAuth2 buttons when `Config.OAuth2Buttons` is empty.
- **OAuth2 auto-detection + display names**: `oauth2.Provider.Names()` returns sorted provider names. `loginpage.ProviderDisplayName()` maps common providers to user-friendly labels (google→Google, github→GitHub). `loginpage.OAuth2ButtonFromProvider(name)` creates a button with auto-generated label.

## Key Gotchas

### Module & Build

1. **GOWORK=off required**: `go.work` covers root + adminui + loginpage + usermgmt + integration_test + examples. `GOWORK=off` needed for CI/commands using per-module go.mod. **Note**: When running `go mod tidy` under `GOWORK=off`, the root module requires the `-e` flag (`go mod tidy -e`) because go-cqrs-lite's `event/v4` go.mod lists an unresolvable `eventtest` dependency (upstream publishing bug — not published as a standalone module). The `-e` flag tolerates the error; build and test work normally. Submodules are not affected. In workspace mode (`go.work`), no `-e` is needed
2. **Module path casing**: go-cqrs-lite uses lowercase `github.com/larsartmann/go-cqrs-lite` (not `LarsArtmann`)
3. **go-cqrs-lite v4.0.0**: Per-module tags (`command/v4.0.0`, `event/v4.0.0`, etc.) published. All modules aligned to v4.0.0. v4 is a clean major-version bump from v3 with backward-compat aliases (`event.AggregateType = id.AggregateType`, etc.) and a restructured module layout (metadata extracted from event, projection extracted from event, id/types consolidated). **PUBLISHING BUG**: go-cqrs-lite v4.0.0 go.mod files reference internal sibling modules with zero pseudo-versions (`v4.0.0-00010101000000-000000000000`) due to local replace directives not being resolved at publish time. Consumers must explicitly `go get` ALL transitive go-cqrs-lite modules at v4.0.0 to override the zero versions
...
9. **encoding/json/v2 is INTENTIONAL**: The project uses `encoding/json/v2` across 28+ .go files. The build **requires** `GOEXPERIMENT=jsonv2` — without it, `go build` fails. This is a deliberate design choice, NOT a mistake. Never add depguard rules to ban `encoding/json/v2` or `encoding/json/jsontext`. BuildFlow auto-detects json/v2 usage and sets `GOEXPERIMENT=jsonv2` for Go toolchain commands. All manual `go build`/`go test` commands must include `GOEXPERIMENT=jsonv2`.
...
- **`CatchUpSubscriber`**: Intentionally NOT adopted. `StartProjections` uses synchronous journal replay + `bus.SubscribeAll` for read-your-writes consistency (ADR-0016). CatchUpSubscriber is asynchronous (per-topic goroutines, channel-based) and pairs with projectionhost. The synchronous design is deliberate — see ADR-0016 for rationale.
- **go-cqrs-lite v3.7.0 adoption audit** (2026-07-07):
  - **`dedup.Ring` ADOPTED**: `es_projection_setup.go` now uses `dedup.Ring` (1024 entries, ~90KB) for replay→live dedup instead of unbounded `map[id.EventID]struct{}`. Memory is O(1) regardless of journal size. Same Ring used internally by `watermill.CatchUpSubscriber`.
  - **`codec.ForEncoding` ADOPTED**: `unmarshalPayload` now resolves codec per-event via `codec.ForEncoding(evt.Encoding())` instead of hardcoded `json.Unmarshal`. Consumers who set `event.DefaultCodec = codec.CBORCodec{}` get transparent CBOR support. Mixed JSON+CBOR event streams decode correctly.
  - **`projectionhost` NOT adopted**: Intentionally dropped projection.Runner complexity per ADR-0016. projectionhost brings crash-restart, DLQ, health/liveness — the current design prefers simplicity (log + continue). Consumers needing retry semantics wrap their projection's Handle method.
