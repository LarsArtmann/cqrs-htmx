...
- **adminui offline sync is now served from the root module:** `sync-worker.js` and the new `sync-client.js` live in the root module's `sync/` directory and are served via `cqrshtmx.SyncWorkerHandler()` and `cqrshtmx.SyncClientHandler()`. adminui delegates to these handlers — the old `adminui/assets/sync-worker.js` is gone. Any consumer (not just adminui) can now wire offline sync with two handler mounts + one `<script>` tag. The sync-client.js auto-initializes on DOMContentLoaded if `<body data-sse-url>` is present.
