...
- **identity-model is the domain source of truth:** `identity-model/v4` contains ALL pure domain types (IDs, events, commands, states, fold functions, Authz engine, sessions, errors). usermgmt re-exports everything via type aliases (`type Authz = identitymodel.Authz`). When adding a new domain type, add it to identity-model and create the alias in usermgmt. Fold functions stay in usermgmt because they depend on the upcaster registry. Command dispatch uses accessor methods (`c.Email()`) not direct field access (`c.email`).
