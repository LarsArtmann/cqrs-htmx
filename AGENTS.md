...
- **adminui offline queue uses IndexedDB (v4.2.x, Phase 2b / ADR-0040):** `sync-worker.js` persists failed command envelopes to IndexedDB (`cqrshtmx-sync` DB, `commands` store), drains on spawn for cross-session retry, deletes on ACK. Degrades to in-memory when IDB unavailable (private browsing). The `rebuildAndRetry` cross-session path is untested in a real browser — adminui unit tests verify the protocol, not the rendered output.
- **BuildFlow pre-commit hook + `golangci-lint --fix` re-formats staged files:** `.git/hooks/pre-commit` runs `buildflow --build-mode pre-commit --staged-only` then `git add`s the result. If you intentionally wrote code that fights the fixers (e.g. `captured = ctx` to avoid shadowing an outer var, or a multi-`data:` SSE line that dupword considers a duplicate), the hook WILL normalize it back to the buggy form (`captured := ctx`, single `data:` line) and silently re-introduce the bug. **Verified:** running `golangci-lint run --fix` on this repo reverts the anti-shadow fixes in `hooks_test.go`, `ws_dispatch_test.go`, and the multi-newline SSE correction in `sse_event_test.go` — exact fixer not pinned, but the behavior is reproducible. Workarounds: (a) `git commit --no-verify` for such fixes (then re-verify HEAD via tests), (b) `git restore <file>` to undo unwanted fixer reverts, (c) NEVER run `golangci-lint run --fix` repo-wide. Do NOT bypass on commits that don't touch this pattern.
