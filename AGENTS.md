...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| go-cqrs-lite v3.5.0          | CQRS dispatch, pagination, event sourcing (decider, storage/memory, watermill bus, SQL view stores, typed metadata) | All modules                  |
...
- **Idempotency**: `idempotency.go` is now thin backward-compatible aliases over `go-cqrs-lite/idempotency/v3` (now at v3.5.0). Re-exports `IdempotencyStore` (= `idempotency.Store`), `MemoryIdempotencyStore` (= `idempotency.MemoryStore`), `ErrDuplicateCommand` (= `idempotency.ErrDuplicate`), and `NewMemoryIdempotencyStore` wrapper. `CheckAndRecord(ctx, key, ttl)` atomicity (single lock in-memory, future `SET NX` for Redis, `INSERT ON CONFLICT` for SQL). `ErrDuplicateCommand` maps to HTTP 409 Conflict. NOT auto-wired — consumers opt in via `BeforeDispatchHook` or middleware. See `docs/adr/0026-command-idempotency-store.md`.
...
1. **GOWORK=off required**: `go.work` covers root + adminui + usermgmt + integration_test + examples. `GOWORK=off` needed for CI/commands using per-module go.mod. **Note**: `go.work` has a `replace` directive for `event/v3/eventtest` (go-cqrs-lite bug — the eventtest tag is misnamed `event/v3/eventtest/v3.5.0` instead of `event/eventtest/v3.5.0`, making it unresolvable via `go mod download`). The local copy lives in `.vendor-local/eventtest/`. This only affects workspace development, NOT consumers
2. **Module path casing**: go-cqrs-lite uses lowercase `github.com/larsartmann/go-cqrs-lite` (not `LarsArtmann`)
3. **go-cqrs-lite v3.5.0**: Per-module tags (`command/v3.5.0`, `event/v3.5.0`, etc.) published. Modules declare the latest tag per module (most at v3.5.0; `dispatcher/v3` at v3.3.0 — not bumped). v3.5.0 adds CBOR codec support (`codec.CBORCodec`, `event.DefaultCodec`, `event.DecodePayloadAuto`), stricter encoding validation, storage package restructure (`view/`, `relational/` subdirs with backward-compatible aliases), lint cleanup across 10 modules. v3.4.0 added managed projection host (`projectionhost`), durable scheduling, scenario-testing DSL. v3.3.0 added `command.Command.ID()` (embed `command.BasicCommand`), `event.Projection` moved to `projection/`, SQL dead-letter store. v3.1.0 added SQL-backed view stores, typed metadata, multi-DB split presets
...
6. **LSP vs CLI discrepancy**: LSP may show stale warnings after golangci.yml changes; CLI (`golangci-lint run`) is authoritative. Root module reports 0 issues. usermgmt has 2 pre-existing wrapcheck findings in `errors.go` (`cqrshtmx.WithHTTPStatus` returns)
