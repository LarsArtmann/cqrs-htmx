...
- **storage/v4 re-pinned to v4.6.0 (broken-tag mitigation):** published `storage/v4 v4.7.0` does not compile (`sql/keyset.go:43` assigns an undeclared `err`), breaking hermetic builds after the 08-16 dep sweep; usermgmt, adminui, loginpage, setup, and systemadapter now require the last-good v4.6.0 with suppression-commented justifications. Upstream follow-up tracked in TODO_LIST (P1).
