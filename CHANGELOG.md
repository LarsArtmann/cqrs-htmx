...
- **`datastar/v4.8.0` release train (local, pending push)**: upstream unblocked — go-datastar `static/v0.2.0` tagged — so the go-datastar-family dev-replaces were stripped from `datastar` (all 3; the module now has zero replaces), `examples/datastar-demo` + `integration_test` (3 upstream replaces each; the `cqrs-htmx/datastar/v4` family replaces remain until the tag is pushed), and `health` (the `go-datastar/static` replace and its comment block). Local signed tag `datastar/v4.8.0` cut at the verified strip commit; all modules verified hermetically (`GOWORK=off go mod tidy && go build ./... && go vet ./...`).
