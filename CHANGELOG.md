...
- **Offline sync extracted to root module** (`sync/sync-worker.js`, `sync/sync-client.js`, `sync_embed.go`, `sync_serve.go`): The offline command queue SharedWorker and tab-side sync client are no longer trapped inside adminui. They are now general-purpose root module assets served via `cqrshtmx.SyncWorkerHandler()` and `cqrshtmx.SyncClientHandler()`, following the same pattern as `HTMXScriptHandler()`. Any consumer (not just adminui) can now wire offline sync with two handler mounts + one `<script>` tag. adminui's `admin.js` was slimmed from ~400 to ~65 lines (CSRF, sidebar, toast, confirm only) — all sync coordination (SSE, SharedWorker, HTMX events, indicator, retry) moved to `sync-client.js`. The old `adminui/assets/sync-worker.js` is deleted; adminui delegates to root handlers.
