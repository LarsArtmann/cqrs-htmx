...
- **Projection status handler** (`projection_status_handler.go`): `ProjectionStatusHandler(provider)` serves live projection health as JSON with `no-cache` semantics and per-request ETag. `ProjectionStatusProvider` interface (`ProjectionStatuses() []ProjectionStatusEntry`) implemented by `*usermgmt.Service` and `*usermgmt.EventSourcedSetup`. Enables ops dashboards and alerting on projection lag/failures.
- **usermgmt event catalog** (`usermgmt/es_event_catalog.go`): `DefaultEventCatalog()` pre-populates all 21 event types across User (12), Membership (3), Tenant (4), and Bot (2) aggregates with descriptions and payload field metadata. `EventCatalog()` method on both `EventSourcedSetup` and `Service`.
- **usermgmt projection health and rebuild** (`usermgmt/es_projection_health.go`): `ProjectionStatuses()` and `RebuildProjection(ctx, name)` methods on both `EventSourcedSetup` and `Service`. `RebuildProjection` stops the host, resets the named projection's checkpoint and read-model state, then creates a fresh host that replays the entire journal — preserves read-your-writes by blocking until all workers reach live state.
- **Consistency model documentation** (`docs/guides/consistency-model.md`): Documents read-your-writes at startup, causal consistency per aggregate, and what is NOT guaranteed (cross-projection atomicity, multi-instance, monotonic reads during rebuild).
- **Event replay and rebuild guide** (`docs/guides/event-replay-and-rebuild.md`): `host.Reset()` mechanics, `RebuildProjection` API, projection name table, when to/not to rebuild.
- **Auth provider fault tolerance guide** (`docs/guides/auth-provider-fault-tolerance.md`): Transient vs permanent failures, gobreaker wrapper examples for OAuth2 and WebAuthn, circuit breaker config.
- **Event store storage health guide** (`docs/guides/event-store-storage-health.md`): Immutable event log principle, snapshotting for read performance, Postgres VACUUM/partitioning, SQLite WAL checkpointing, monitoring queries, capacity planning, explicit "What NOT to Do" section.
- **Event catalog guide** (`docs/guides/event-catalog-guide.md`): How to serve/extend/consume the event catalog, all 21 events listed, merging custom events with usermgmt catalog.
- **Projection health monitoring guide** (`docs/guides/projection-health-monitoring.md`): Status endpoint reference, field reference, status values, lag interpretation, alerting examples (Prometheus, HTTP polling).
- **Rebuild projection runbook** (`docs/guides/rebuild-projection-runbook.md`): When to rebuild, prerequisites, procedure, verification, troubleshooting.
- **ROADMAP v5 decomposition plan** (`ROADMAP.md`): Added "v5 Vision: usermgmt Decomposition (Deferred)" section with module boundaries, decomposition trigger criteria, proposed module tree, cost/benefit analysis.

### Changed

- **Projection host factory deduplicated** (`usermgmt/es_projection_setup.go`, `usermgmt/es_projection_health.go`): Extracted shared `startProjectionHost` helper used by both `StartProjections` (initial startup) and `RebuildProjection` (rebuild after reset). Eliminates ~50 lines of duplicated seekable-check + host-creation + register + start + drain logic. The `createProjectionHost` wrapper now delegates to the shared helper after converting `event.Store` to `event.Journal`.
- **Service field renamed** (`usermgmt/service_core.go`): `projectionListField` → `projections` for consistency with `EventSourcedSetup.projections`. No API change — internal field.

- **identity-model module** (`identity-model/v4`): Extracted pure domain-model module for event-sourced identity management. Contains ALL domain types (UserID, TenantID, BotID, ActorID, Session, User, Membership, ExternalAccount, WebAuthnCredential), event payloads (22 structs), commands (19 structs with accessor methods), state structs + fold functions (FoldUser/FoldMembership/FoldTenant/FoldBot), Authz engine (Casbin-backed with Enforce/EnforceEx/Authorize/RolesForUser/AddPolicy/Apply), RBAC model + default policies + role hierarchy, domain errors (errorfamily-only, no HTTP dependency), crypto helpers (GenerateToken/HashToken/VerifyToken), upcaster registry (Upcaster/UpcasterRegistry/SetUpcasterRegistry), exported constants (41 event/command/aggregate-type constants, MaxEmailLength, ActorKindUserStr/BotStr, CurrentSchemaVersion), UnmarshalPayload, and ActorKindFromString. 40 tests (36 original + 4 new: FoldMembership lifecycle, FoldBot lifecycle, ActorID JSON round-trip, Session JSON round-trip). Casbin is a first-class dependency.

### Changed

- **usermgmt wired to identity-model** (`usermgmt/`): All domain type definitions replaced with type aliases to identity-model (`type Authz = identitymodel.Authz`, `type UserID = identitymodel.UserID`, etc.). Method-bearing types (Authz, ActorID, ActorKind, UserState, etc.) inherit all methods through the alias. Command dispatch uses accessor methods (`c.Email()`, `c.Roles()`) instead of direct field access. Errors are re-exported with `cqrshtmx.WithHTTPStatus` wrapping where the correct HTTP status differs from the errorfamily default. Fold functions, upcaster registry, and all constants now live in identity-model — usermgmt aliases them via `var` (Go has no const alias). The split brain between identity-model and usermgmt is fully eliminated — identity-model is the single source of truth for domain types, fold logic, and constants.
- **go-cqrs-lite API updates propagated** (workspace-wide): `AggregateID()` → `StreamID()` on commands, `AggregateType` → `StreamType` on events, `event.AggregateRef` → `id.StreamRef`, `snapshot.AggregateID/Type` → `snapshot.StreamID/Type`.
- **Constants consolidated to identity-model** (`identity-model/constants.go`, `usermgmt/es_constants.go`): All 41 event/command/aggregate-type constants exported from identity-model (e.g. `EventUserRegistered`, `CmdRegisterUser`) and aliased via `var` in usermgmt. `MaxEmailLength`, `ActorKindUserStr`/`ActorKindBotStr`, `ActorKindFromString`, `CurrentSchemaVersion` also consolidated. Dead `maxEmailLength` removed from usermgmt.
- **Fold functions and upcaster registry consolidated to identity-model** (`identity-model/fold.go`, `identity-model/upcaster.go`, `identity-model/payload.go`): Upcaster registry (`Upcaster`, `UpcasterRegistry`, `NewUpcasterRegistry`, `SetUpcasterRegistry`) moved from usermgmt to identity-model. identity-model's `applyUpcasters` is now real (was a no-op pass-through). `UnmarshalPayload` exported from identity-model. usermgmt's fold functions replaced with `var foldUser = identitymodel.FoldUser` aliases. `es_upcaster.go` and `es_upcaster_test.go` deleted from usermgmt (tests moved to `identity-model/upcaster_test.go`). `unmarshalPayload` kept as a thin 3-line wrapper in usermgmt (Go cannot alias generic functions via var).
- **identity-model added to flake.nix** (`flake.nix`): identity-model now included in all 7 flake targets (test, test-race, test-flake, test-fuzz, lint, coverage, build).
- **ADRs** (`docs/adr/`): Added ADR-0043 (identity-model extraction) and ADR-0044 (Casbin as first-class dependency of identity-model).

- **Offline sync extracted to root module** (ADR-0042) (`sync/sync-worker.js`, `sync/sync-client.js`, `sync_embed.go`, `sync_serve.go`): The offline command queue SharedWorker and tab-side sync client are no longer trapped inside adminui. They are now general-purpose root module assets served via `cqrshtmx.SyncWorkerHandler()` and `cqrshtmx.SyncClientHandler()`, following the same pattern as `HTMXScriptHandler()`. Any consumer (not just adminui) can now wire offline sync with two handler mounts + one `<script>` tag. adminui's `admin.js` was slimmed from ~400 to ~65 lines (CSRF, sidebar, toast, confirm only) — all sync coordination (SSE, SharedWorker, HTMX events, indicator, retry) moved to `sync-client.js`. The old `adminui/assets/sync-worker.js` is deleted; adminui delegates to root handlers.
- **Sync JS modernized** (`sync/sync-worker.js`, `sync/sync-client.js`): Modernized all `var` declarations to `const`/`let`, added `VERSION = "1.1.0"` constant to both files for DevTools debugging, bumped `syncVersion` from `1.0.0` to `1.1.0` (ETag cache-bust for the content change). Added `CONFIGURATION` comment block documenting how to customize retry limits via `SyncWorkerHandlerWith`. Added `@ts-check` pragma and JSDoc on all public functions in both JS files. Added `data-sync-worker-url` attribute override on `<script>` tag for custom worker paths.
- **Sync ETag prefix unified** (`sync_serve.go`): Changed ETag format from `"cqrshtmx-sync-worker-%s"` / `"cqrshtmx-sync-client-%s"` to `"sync-worker-%s"` / `"sync-client-%s"` — consistent with the bare `"htmx-%s"` prefix pattern used by `HTMXScriptHandler`.
...
- **Sync handler `With` variants** (`sync_serve.go`): Added `SyncWorkerHandlerWith(js, version)` and `SyncClientHandlerWith(js, version)` — mirrors the `HTMXScriptHandlerWith` pattern, allowing consumers to serve custom sync-worker.js or sync-client.js with different retry configuration. The zero-arg `SyncWorkerHandler()` / `SyncClientHandler()` now delegate to these.
- **`data-sync-worker-url` override** (`sync/sync-client.js`): Consumers who mount sync-worker.js at a different path than sync-client.js can specify the worker URL via a `<script data-sync-worker-url="/path">` attribute instead of relying on the default path-derivation logic.
- **Sync handler godoc examples** (`example_htmx_test.go`): Added `ExampleSyncWorkerHandler`, `ExampleSyncClientHandler`, `ExampleSyncClientScriptTag`, `ExampleSyncVersion` for godoc discoverability.
- **adminui delegation integration tests** (`adminui/coverage_gaps2_test.go`): Added `TestPanel_SyncWorkerDelegationMatchesRoot` and `TestPanel_SyncClientDelegationMatchesRoot` verifying adminui's delegation to root handlers serves byte-identical content — catches drift if someone replaces the delegation with a stale embedded copy.
- **Sync version-drift prevention test** (`sync_serve_test.go`): Added `TestSyncVersionMatchesJSConstants` — reads `VERSION` constants from the embedded JS files and asserts they match Go `syncVersion`. Prevents silent cache-stale bugs when JS content changes but the version isn't bumped.
- **XSS doc on SyncClientScriptTag** (`sync_serve.go`): Documented that the `path` parameter is developer-controlled, not user input. If it originates from untrusted input, sanitize first to prevent attribute injection.
- **Type-safe command/query HTTP handlers and typed/validating decoders** (`app.go`, `handler.go`, `options_decode.go`, `typed_handlers_test.go`): Added `cqrshtmx.CommandTyped[Q](app, type, opts...)` and `cqrshtmx.QueryTyped[Q, R](app, type, opts...)` — package-level generic functions (Go does not allow generic methods) that build type-safe HTTP handlers. Pair with `DecodeJSONTyped[Q]` / `DecodeJSONQueryTyped[Q]` / `DecodeFormTyped[Q]` / `DecodeFormQueryTyped[Q]` and `RenderJSON[R]()`. Also added `DecodeAndValidateJSON[T]` / `DecodeAndValidateJSONQuery[T]` / `DecodeAndValidateForm[T]` / `DecodeAndValidateFormQuery[T]` for one-step decode + `T.Validate()`. Type mismatches are returned as 400 Bad Request; typed handlers share the full auth/CSRF/timeout/hook pipeline. Internally, `dispatchRequest` was extracted from a method to a standalone generic function shared by both typed and untyped handlers.
...
- **Offline command queue — Phase 2b (IndexedDB persistence, ADR-0040)** (`sync/sync-worker.js`): The SharedWorker offline queue now persists command envelopes to IndexedDB so queued mutations survive closed tabs and browser restarts. On spawn it drains persisted commands and broadcasts retry to every connected tab (cross-tab/cross-session retry); deletes on ACK; degrades gracefully to in-memory when IndexedDB is unavailable (private browsing / quota). All persistence is confined to JavaScript assets (now in the root module) — the Go library has zero new client-side persistence concerns. Reverses ADR-0030 (now SUPERSEDED by ADR-0040).
- **Offline command queue — production hardening** (`sync/sync-worker.js`, `sync/sync-client.js`): Eliminated the parallel in-memory queue / IDB split (IDB is now the single source of truth). Fixed double-retry race (flush + drainPersisted both firing on connect). Fixed port leak (dead ports never removed from array → now tracked by tabId in a Map with hello/bye protocol and postMessage-failure cleanup). Fixed null-envelope poison commands (now rejected as dead before persisting). Added retry count (max 10) and TTL (24h) eviction so failed commands don't retry forever. Added staggered retry delivery (100ms per command, capped 2s) and targeted retry (originating tab preferred, round-robin fallback) to prevent thundering herd. Added `store.count()` for pending-count queries (was full `getAll().length` scan). Added in-memory fallback Map for when IndexedDB is unavailable. Added flush dedup (concurrent flush calls coalesced). Tab side: generates tabId, sends hello/bye, handles dead messages.
...
- **OpenAPI 3.1 spec generation** (`openapi/`, `options_openapi.go`): New dependency-free, fluent builder for OpenAPI 3.1 documents so consumers can declare and serialize their API spec explicitly (cqrs-htmx cannot introspect a consumer-owned router). The `openapi` package provides JSON-Schema-draft-2020-12 types (`Schema` with `String`/`Integer`/`Object`/`Array`/`Ref`/`False`/`ErrorSchema`), a fluent builder (`New(title, version).Path(url, ops...).Schema(name, schema)` with per-method `Get`/`Post`/`Put`/`Patch`/`Delete`/`Head`/`Options`), and `Spec.JSON()` serialization via `encoding/json/v2`. Root integration: `WithOpenAPI(op)` attaches operation metadata to a handler (pure documentation — no runtime effect on dispatch), and `OpenAPISpecHandler(spec) (http.HandlerFunc, error)` serves the spec at `/openapi.json`. The spec is serialized once at construction — eager, so any serialization error surfaces at startup and the returned handler is immutable and concurrency-safe (no lazy caching, no locks). Responses carry a stdlib FNV-1a ETag, a 1-year immutable `Cache-Control`, and 304-on-`If-None-Match`.
- **Admin UI: OAuth2 external-account views** (`adminui`): The user detail page now shows a card listing linked external (OAuth2/OIDC) accounts with provider badge, email, link date, and an Unlink button. Configured-but-unlinked providers appear as badges in a footer with help text explaining that linking is user-side. Unlinking calls `Service.UnlinkExternalAccount` (which enforces the last-auth-method guard). Linking is intentionally admin-side-impossible: the OAuth2 handshake requires the user's own authentication and cannot be impersonated. Adds `usermgmt.NewExternalAccount(...)` public constructor (the embedded core is unexported, so external callers — including tests — cannot build a struct literal).
- **SSE broadcaster load benchmarks** (`sse_broadcaster_bench_test.go`): Five leak-safe benchmarks measuring broadcaster fan-out across {1, 10, 100, 1000} subscribers, slow-consumer drop rate, and subscribe/unsubscribe churn. Replaces the old goroutine-leaking benchmarks (`BenchmarkBroadcasterBroadcastStress`, `BenchmarkBroadcasterConcurrentSubscribe`) removed from `benchmark_server_test.go`.
- **Property-based tests for event folds** (`usermgmt/property_sequences_test.go`, `property_sequences_more_test.go`): 13 sequence-property tests verifying that event-sourced folds are well-behaved across random event streams — associativity (grouping order doesn't matter), tombstone idempotency (re-tombstoning is a no-op), unknown-event-halts, and aggregate-specific invariants for User, Tenant, Membership, and Bot.
- **dedup.Ring benchmark — 100K tier** (`usermgmt/es_dedup_benchmark_test.go`): Extended the dedup ring-vs-map benchmark from {100, 1K, 10K} to include 100K events with `b.ReportAllocs()` on all tiers, confirming O(1) bounded-memory behavior at high journal volumes.
