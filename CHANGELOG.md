...
- **Datastar adapter module** (`datastar/v4`): Optional, fully-isolated Go module (`github.com/larsartmann/cqrs-htmx/datastar/v4`) that lets consumers use [Datastar](https://data-star.dev/) instead of (or alongside) HTMX for frontend reactivity. Depends only on `datastar-go` SDK + `go-cqrs-lite/event/v4` — zero transitive deps from root (no casbin, httputil, or go-sse). Features: self-hosted `datastar.js` v1.0.2 with ETag/1yr cache (`ScriptHandler`/`ScriptHandlerWith`/`ScriptTag`), `ReadSignals` decoder, fluent `Response` builder (PatchElements/PatchSignals/Redirect/ExecuteScript), `Broadcaster` with bounded patch ring buffer replay (default 256 patches, reconnects via `Last-Event-ID`), `EventBridge` for declarative domain-event-to-patch mapping, and ~50 SDK option re-exports for single-import convenience. 57 tests, 95.1% coverage (gate 90%), 0 lint issues. See `docs/guides/datastar-integration.md` and ADR-0045.
...
- **Coverage numbers updated**: Root 93.7% (gate 90%), openapi 99.0%, usermgmt 81.6% (gate 74%), identity-model 74.9% (gate 70%), dashboardui 84.0% (gate 60%). All 9 coverage gates pass.
